...
- **Supported-market boundary:** Limitless is ingested broadly, but only markets whose source shape matches an existing deterministic engine can receive an oracle probability. Today that means named national-team 2026 FIFA World Cup outright winner children can run through `engines/sports.py`; the catch-all "Other" child and all stages/matchups/props are included as non-actionable records. Limitless economics markets are included as non-actionable records because current economics support is core CPI, while the live Limitless inflation examples are headline CPI/GDP/Fed/recession shapes. Limitless weather is a first-priority future route when a live market exposes station/source/date/metric/strike fields cleanly enough for `engines/weather.py`.
...
- **Phase 8 verification, 2026-07-09:** `python3 verify.py --phase 8` passed with broad ingestion enabled. It scanned **644** live markets in the gate run: venue counts `kalshi=192`, `polymarket=120`, `limitless=332`; included **332** records, including **245** non-actionable unsupported domain records; evaluated **87** records with existing engines; and skipped only **311** `other`/price-oracle records. Coverage status counts were `actionable=40`, `wait=39`, `fee_missing=8`, `source_missing=22`, `parse_missing=48`, `model_missing=175`. The gate now fails if coverage fields are missing, Limitless is not read, grouped markets are not flattened, unsupported Limitless domain shapes are not included as non-actionable records, or priced Limitless records become actionable while the fee gap remains.
- **Default artifact run, 2026-07-09:** `PYTHONPATH=src python3 -m rwoo.scanner --write --top 30` scanned **2,170** live markets, included **920** weather/economics/sports records, marked **764** included records as non-actionable coverage gaps, skipped **1,250** `other` records, and found **41** actionable candidates. The public artifact now surfaces included unsupported reasons such as missing tennis/NBA/NHL/Fed/GDP/headline-CPI engines instead of silently hiding those markets.

### Ledger status as of 2026-07-09 (Phase 6 + pre-listing hardening + Limitless read-only scanner complete)
All GATE 0–8-required facts are verified with real evidence above, including a real, correctly-verified X Layer mainnet anchor transaction (§16.1) after catching and fixing a genuine false-positive in the anchor verification logic itself. Other open items remain explicitly flagged and **not assumed**: the Payment SDK settlement token (§7), the Kalshi fee-schedule PDF direct fetch blocked by HTTP 429 from this workspace, exact Limitless fee/execution support (§21), future weather/economics/sports venue expansion, funded execution/risk controls, and the hosted public calibration page.
