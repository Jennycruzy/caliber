...
- **Verified:** WebSearch + WebFetch, 2026-07-08. The primary page `bls.gov/schedule/news_release/cpi.htm` returned HTTP 403 to direct fetch from this workspace. Cross-referenced instead via `usinflationcalculator.com/inflation/consumer-price-index-release-schedule/`, which states it reproduces the official BLS schedule and lists exact dates through December 2026, including two real disruptions handled explicitly rather than smoothed over: the October 2025 release was **canceled** (government shutdown) and the November 2025 release was rescheduled to December 18, 2025 (from December 10).
...
- **Status:** today's real daily quota was already exhausted by earlier testing in this workspace before the caching fix landed. `BLS_API_KEY` env var support was added (free registration at `data.bls.gov/registrationEngine/` raises the limit to ~500/day) — **registration requires solving a CAPTCHA, which cannot be automated or bypassed**; this needs the Operator to complete it directly. Until then or until the daily quota resets, the economics backtest sample size is capped by this real external limit, disclosed honestly in `verify.py --phase 5`'s output rather than hidden.
- **Gate behavior after audit, 2026-07-08:** direct fetches of the official BLS flat-file mirror at `https://download.bls.gov/pub/time.series/cu/` also returned HTTP 403 from this workspace, so there is no primary-source fallback available here today. The Phase 5 harness now treats economics as **honestly quota-blocked/incomplete** when BLS refuses data: it does not print a fake reliability curve or Brier score, and it keeps production economics in `docs/BUILD_GAPS.md`.
...
### Ledger status as of 2026-07-08 (Phase 6 complete, including a real corrected X Layer mainnet anchor)
All GATE 0–6-required facts are verified with real evidence above, including a real, correctly-verified X Layer mainnet anchor transaction (§16.1) after catching and fixing a genuine false-positive in the anchor verification logic itself. Other open items remain explicitly flagged and **not assumed**: the Payment SDK settlement token (§7), the primary Kalshi fee-schedule PDF blocked by Vercel (§13), the economics consensus-forecast distribution (§12), production-grade sports model inputs (§14), and broader domain calibration (§15).
