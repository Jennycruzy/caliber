...
- **Phase 8 verification, 2026-07-09:** `python3 verify.py --phase 8` passed with Limitless enabled. It scanned **445** live markets total: venue counts `kalshi=72`, `polymarket=80`, `limitless=293`; evaluated **88** supported records; skipped **357** unsupported records; flattened **219** Limitless grouped children; and had **0** item-level errors. Limitless skip reasons were counted explicitly: other/price-oracle unsupported `79`, sports unsupported shape `160`, headline CPI not supported by core-CPI engine `9`, economics unsupported shape `37`. The gate now fails if Limitless is not read, grouped markets are not flattened, unsupported Limitless shapes are not counted as skips, or any Limitless record becomes actionable while the fee gap remains.
